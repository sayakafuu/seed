const TANE_VERSION = "v9";

const COLORS = ["#F95F86", "#FF7D93", "#FF9E8A", "#FFB46F", "#FFD06F", "#A9CBA0", "#7ED6C9", "#B985E8"];
const STORAGE_KEY = "tane_v6_plan_queue";
const DEFAULT_CATEGORY_COLORS = ["#F95F86", "#7ED6C9", "#B985E8", "#FFD06F"];
const DEFAULT_CATEGORIES = ["やること", "行きたい・食べたい", "気になる", "待ち"].map((name, i) => ({
  name,
  color: DEFAULT_CATEGORY_COLORS[i] || COLORS[i % COLORS.length]
}));
...
const state = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null") || {
  ...
  cards: [
    ...
    { title: "台湾旅行", category: "行きたい・食べたい", current: "行き先を決める", memo: "", history: [], queue: ["宿を決める", "宿を予約する", "行くところを決める", "予定を組む"], color: COLORS[2] },
    { title: "図鑑", category: "気になる", current: "本屋で見てみる", memo: "友達おすすめ", history: [], queue: [], color: COLORS[4] }
  ]
};
...
function migrateState() {
  if (!Array.isArray(state.categories)) state.categories = DEFAULT_CATEGORIES.map(x => ({ ...x }));
  if (typeof state.categories[0] === "string") {
    state.categories = state.categories.map((name, i) => ({ name, color: DEFAULT_CATEGORY_COLORS[i] || COLORS[i % COLORS.length] }));
  }
  state.categories = state.categories.map((cat, i) => ({
    name: String(cat.name || `カテゴリ${i + 1}`).trim(),
    color: cat.color || COLORS[i % COLORS.length]
  })).filter(cat => cat.name);
  if (!state.categories.length) state.categories = DEFAULT_CATEGORIES.map(x => ({ ...x }));
  if (!Array.isArray(state.archive)) state.archive = [];
  if (!Array.isArray(state.cards)) state.cards = [];
  state.cards.forEach(card => {
    normalizeCard(card);
    if (!state.categories.some(cat => cat.name === card.category)) {
      state.categories.push({ name: card.category || "未分類", color: card.color || COLORS[0] });
    }
  });
...
function normalizeCard(card) {
  ...
  if (!card.category) card.category = state.categories[0]?.name || "やること";
...
function render() {
  ...
  state.categories.forEach(category => {
    const cards = state.cards.filter(card => card.category === category.name);
    if (!cards.length) return;
    const section = document.createElement("section");
    section.className = "group";
    section.style.setProperty("--groupColor", category.color);
    section.innerHTML = `<div class="groupHead"><div class="groupName">${h(category.name)}</div></div><div class="list"></div>`;
...
function createCard(card, index) {
  ...
  let moved = false, tapCancelled = false, longPressed = false;
  let timer = null;

  item.addEventListener("touchstart", e => {
    startX = e.touches[0].clientX;
    startY = e.touches[0].clientY;
    dx = 0; dy = 0; moved = false; tapCancelled = false; longPressed = false;
  ...
  action.onclick = () => { nextIndex = index; openNext(); };
  return row;
}
...
function openEdit(index, category) {
  ...
  const card = index === null
    ? { title: "", category: category || state.categories[0].name, current: "", memo: "", history: [], queue: [], color: categoryColor(category || state.categories[0].name) }
    : normalizeCard(state.cards[index]);
  selectedColor = card.color || categoryColor(card.category) || COLORS[0];
  const historyHtml = card.history.length ? `<details class="historyWrap"><summary>これまで</summary><div class="history">${card.history.map(x => `<div>✓ ${h(x)}</div>`).join("")}</div></details>` : "";
  ...
  drawColorButtons("colors", selectedColor, color => { selectedColor = color; drawColorButtons("colors", selectedColor, arguments.callee); vibrate(6); });
  editDialog.showModal();
  if (document.activeElement && document.activeElement.blur) document.activeElement.blur();
}

function drawColorButtons(containerId, activeColor, onSelect) {
...
function saveEdit() {
  ...
  const card = {
    ...
    color: selectedColor || categoryColor(category)
  };
...
function openNext() {
  ...
  nextDialog.innerHTML = `
    <form method="dialog" class="dialogForm nextForm">
      <h2>次にやること</h2>
      <div class="nextContext"><div class="nextItemTitle">${h(card.title)}</div>${card.current ? `<div class="nextDone">✓ ${h(card.current)}</div>` : ""}</div>
      <textarea id="nextInput" placeholder="次にやることを入力">${h(proposed)}</textarea>
      <div class="btns"><button class="btn" type="button" onclick="nextDialog.close()">キャンセル</button><button class="btn ok" type="button" onclick="saveNext()">OK</button></div>
    </form>`;
...
function openFinish() {
  finishDialog.innerHTML = `<form method="dialog" class="finishBox"><button class="finishBtn finishSave" type="button" onclick="finishCard()"><span class="miniChest" aria-hidden="true"></span><span>しまう</span></button><button class="finishBtn finishBack" type="button" onclick="finishDialog.close()"><span class="backIcon" aria-hidden="true">↺</span><span>戻す</span></button></form>`;
...
function finishCard() {
  ...
  if (item && rect) {
    ...
    save(); render(); sparkleFlowFromRect(null);
  }
}

function showArchive() {
  archiveDialog.innerHTML = `<form method="dialog" class="dialogForm archiveForm"><div class="archiveTop"><button type="button" class="backBtn" onclick="archiveDialog.close()">‹</button><h2>保管庫</h2><span></span></div><div id="archiveList"></div></form>`;
  ...
  if (!state.archive.length) {
    ...
    state.archive.forEach(card => {
      ...
      row.innerHTML = `<span class="archiveFlower" aria-hidden="true">✿</span><span class="oldText"><span class="oldTitle">${h(card.title)}</span>${card.current ? `<span class="oldNow">${h(card.current)}</span>` : ""}</span><time class="oldDate">${formatArchiveDate(card.archivedAt)}</time>`;
...
function openCategories() {
  categoryDialog.innerHTML = `<form method="dialog" class="dialogForm categoryForm"><div class="archiveTop"><button type="button" class="backBtn" onclick="categoryDialog.close()">‹</button><h2>カテゴリ</h2><span></span></div><div id="categoryList"></div><button type="button" class="addCategoryBtn" onclick="openCategoryEdit(null)">＋ カテゴリを追加</button><div class="btns single"><button class="btn ok" type="button" onclick="categoryDialog.close()">完了</button></div></form>`;
  drawCategoryList();
...
function saveCategoryEdit() {
  ...
  if (categoryEditIndex === null) state.categories.push({ name, color: selectedColor });
    ...
    state.categories[categoryEditIndex] = { name, color: selectedColor };
    state.cards.forEach(card => { if (card.category === original) card.category = name; });
    state.archive.forEach(card => { if (card.category === original) card.category = name; });
  }
...
function deleteCategoryNow() {
  ...
  state.cards.forEach(card => { if (card.category === removed.name) card.category = moveTo; });
  state.archive.forEach(card => { if (card.category === removed.name) card.category = moveTo; });
...
function sparkleFlowFromRect(rect) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    p.style.animationDuration = `${1.16 + Math.random() * .50}s`;
    document.body.appendChild(p);
    setTimeout(() => p.remove(), 2400);
  }
  archiveBtn.classList.add("receivingLight");
  setTimeout(() => archiveBtn.classList.remove("receivingLight"), 1400);
}

function magicFromRect(rect) { sparkleFlowFromRect(rect); }
function magic() { sparkleFlowFromRect(null); }
