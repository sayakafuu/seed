  ...
  row.innerHTML = `
    ...
    <div class="item">
      <div class="inner" style="--lineColor:${card.color}">
        ...
        <div class="tag">${h(card.category)}</div>
...
function openFinish() {
  finishDialog.innerHTML = `
    <form method="dialog" class="finishBox">
      <button class="btn ok" type="button" onclick="finishCard()">しまう</button>
      <button class="btn" type="button" onclick="finishDialog.close()">戻す</button>
    </form>
  `;
...
function magic() {
  ...
  const fromX = window.innerWidth * 0.55;
  const fromY = window.innerHeight * 0.58;
  for (let i = 0; i < 18; i++) {
    const p = document.createElement("div");
    p.className = "finishSpark" + (i % 6 === 0 ? " softStar" : "");
    const startX = fromX + Math.random() * 90 - 45;
    const startY = fromY + Math.random() * 70 - 35;
    p.style.left = `${startX}px`;
    p.style.top = `${startY}px`;
    p.style.setProperty("--tx", `${toX - startX + Math.random() * 24 - 12}px`);
    p.style.setProperty("--ty", `${toY - startY + Math.random() * 24 - 12}px`);
